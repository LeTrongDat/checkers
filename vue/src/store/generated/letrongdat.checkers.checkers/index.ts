import { Client, registry, MissingWalletError } from 'LeTrongDat-checkers-client-ts'

import { Params } from "LeTrongDat-checkers-client-ts/letrongdat.checkers.checkers/types"
import { StoredGame } from "LeTrongDat-checkers-client-ts/letrongdat.checkers.checkers/types"
import { SystemInfo } from "LeTrongDat-checkers-client-ts/letrongdat.checkers.checkers/types"
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: letrongdat.checkers.checkers initialized!')
		...
		async QueryParams({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.LetrongdatCheckersCheckers.query.queryParams()).data
		...
		async QuerySystemInfo({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.LetrongdatCheckersCheckers.query.querySystemInfo()).data
		...
		async QueryStoredGame({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.LetrongdatCheckersCheckers.query.queryStoredGame( key.index)).data
		...
		async QueryStoredGameAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.LetrongdatCheckersCheckers.query.queryStoredGameAll(query ?? undefined)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.LetrongdatCheckersCheckers.query.queryStoredGameAll({...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
		...
		async sendMsgRejectGame({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.LetrongdatCheckersCheckers.tx.sendMsgRejectGame({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async sendMsgPlayMove({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.LetrongdatCheckersCheckers.tx.sendMsgPlayMove({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async sendMsgCreateGame({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.LetrongdatCheckersCheckers.tx.sendMsgCreateGame({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async MsgRejectGame({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.LetrongdatCheckersCheckers.tx.msgRejectGame({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgRejectGame:Init Could not initialize signing client. Wallet is required.')
		...
		async MsgPlayMove({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.LetrongdatCheckersCheckers.tx.msgPlayMove({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgPlayMove:Init Could not initialize signing client. Wallet is required.')
		...
		async MsgCreateGame({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.LetrongdatCheckersCheckers.tx.msgCreateGame({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgCreateGame:Init Could not initialize signing client. Wallet is required.')
